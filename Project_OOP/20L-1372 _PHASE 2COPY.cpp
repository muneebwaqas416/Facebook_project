...
{
public:
	Memory(char * Text , Base *uptr ,Posts *ptr) : Posts(Text,uptr)
	{
		original_post = ptr;
	}
	~Memory();
...
public:
	...
	void Print()
	{
		cout << Base::getid() << " " << F_name << " " << L_name << endl;  
	}
	void Print_1()
	{
		cout << F_name  << " " << L_name << " ";
...
public:
	...
	void Sharememory(char *ID , Base * , char *);
...
void Facebook::Sharememory(char * ID , Base * uptr , char * Text)
{
	Posts *ptr = Search_post_by_id(ID);
	Memory *mptr = new Memory(Text,uptr,ptr);
	Addpost(mptr);
	uptr->Addposttoyourtimeline(mptr);
...
void Facebook::Viewmemory(User *uptr)
{
	for (int i = 0; i < no_of_posts; i++)
	{
		...
		//User timeline
		char *ID = Helper::Getstringfrombuffer(bptr->getid());
		...
		{
			Date d1;
			d1 = Date::get_currentdate();
			if(Postlist[i]->Shared_date.Getdate() == d1.Getdate() && Postlist[i]->Shared_date.Getdate() == d1.Getdate())
			{
				if(Postlist[i] != nullptr)
				{
					cout << "We hope you enjoy looking back and sharing your memories on Facebook, from the most recent to thoselong ago.";
					cout << endl;
					cout << "On this Day" << endl;
					int date = d1.getyear() - Postlist[i]->Shared_date.getyear();
					cout << date  <<  " Years Ago" << endl;//make 2 a interval
					Postlist[i]->Print();
				}
			}
		}
